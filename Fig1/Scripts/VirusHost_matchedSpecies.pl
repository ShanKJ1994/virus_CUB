...
while (my $line=<IN2>) {
	chomp $line;
	my @mid=split /\t/,$line;
	my @shu=split /\s+|_|\\./,lc($mid[0]);
	$host{"$shu[0]_$shu[1]"}="$shu[0]_$shu[1]";
}
close IN2;
...
while (my $line=<IN2>) {
	...
	if ($mid[2]=~/[A-Z]/i) {
		$mid[2]=lc $mid[2];
		my @shu=split /\s+|_|\./,$mid[2];
		if (exists $host{"$shu[0]_$shu[1]"}) {
			$virus{"$mid[0]:$mid[1]"}{$mid[2]}=$host{"$shu[0]_$shu[1]"};
		}
	}
}
close IN2;
